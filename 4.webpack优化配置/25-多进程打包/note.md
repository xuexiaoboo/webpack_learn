> 24-PWA 复制一份
> 下载 thread-loader `npm i thread-loader -D`  这里用来处理babel-loader
...
多进程打包也是有利有弊的
  - 优点：使用多个进程同时打包，对于数量较多的文件打包减少打包时间
  - 缺点：进程的启动是需要时间的（大概600ms），进程间的通信也是存在时间开销的，所以少量文件的打包使用多进程反而打包时间变得更长

一般 thread-loader 启动的进程数是 电脑处理器核数 - 1，如果想指定开启的进程数，可以通过修改配置
